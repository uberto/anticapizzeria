...
sealed abstract class Event(val pk: String) {
    val created = Instant.now()
    val version = 0
}

sealed class OrderEvent(val key: String): Event("Order-" + key)

sealed class ItemEvent(val key: String): Event("Item-" + key)
...
data class DeliverStarted(val phoneNum: String): OrderEvent(phoneNum)
